...
parser.add_argument('-t', '--alltxtpath', default='data/title/all.txt', type=str,
                    help=('the file that stores all transcripts with utterances separated by \\n'
                          'tokenized file is assumed to be in the same dir with "tknzd_" prefix'))
...
# parser.add_argument('-k', '--tokenizedpath', default='data/title/tknzd_all.txt', type=str)
...
if __name__ == "__main__":
    dir, fname = os.path.split(args['alltxtpath'])
    tknzd_file = os.path.join(dir, 'tknzd_' + fname)
    if not os.path.isfile(tknzd_file):
        print("Cannot fine tokenized scripts, will assume ")
        tknzd_file = args['alltxtpath']
        align_switch = False
