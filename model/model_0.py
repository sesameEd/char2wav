...
class Attention(nn.modules.Module):
    def __init__(self, query_size, num_component=3):
        ...
        self.Q = query_size
        self.N_gm = num_component
        self.ha2gmm = nn.Linear(self.Q, self.N_gm * 3)

    def forward(self, query, T):
        shape_assert(query, (-1, self.Q))
        # _B, _H = prev_decoded.shape
        # shape_assert(prev_attn, (_B, self.T))
        rho, beta, kappa = self.ha2gmm(query).split(self.N_gm, dim=-1)
        ...
        # shape_assert(rho, (query.shape[0], self.N_gm, 1))
        phi = (rho * torch.exp(
                    -beta * (self.kappa - torch.arange(float(T)).view(1, 1, T)) ** 2)
              ).sum(dim=1)
...
class Char2Voc(nn.modules.Module):
    def __init__(self, **kwargs):
        ...
        encoded_size = kwargs.get('encoded_size', 256)
        decoded_size = kwargs.get('decoded_size', encoded_size)
        mlp_sizes = kwargs.get('de2voc_mlp_sizes', [256])
        mlp_i2o = list(zip([decoded_size] + mlp_sizes, mlp_sizes + [self.G]))
        if self.do_layernorm:
            self.embedding = nn.Sequential(nn.Embedding(self.C, self.E-1),
                                           nn.LayerNorm(self.E-1))
        else:
            self.embedding = nn.Embedding(self.C, self.E-1)
        self.encoder = nn.GRU(self.E, encoded_size, bidirectional=True)
        self.attention = Attention(query_size=2 * encoded_size + decoded_size, num_component=3)
        if self.do_layernorm:
            self.decoder = LayerNormGRUCell(encoded_size * 2, decoded_size)
            self.de2voc_mlp = nn.ModuleList([
                nn.Sequential(nn.Linear(*tp), nn.LayerNorm(tp[1])) for tp in mlp_i2o])
        else:
            self.decoder = nn.GRUCell(encoded_size * 2, decoded_size)
            self.de2voc_mlp = nn.ModuleList([nn.Linear(*tp) for tp in mlp_i2o])

    def forward(self, x, char_mask, target, len_seq, is_upper=None, tf_rate=0):
        """
        :x  of shape (_B, _T) tensor of input, sequences of -1-padded indices
        :char_mask: byte tensor where characters from masked words are 0
        :result of shape (_T, _B, _G),
        """
        shape_assert(x, (-1, len_seq))
        _B, _T = x.shape
        x = self.embedding(x) * char_mask.float()
        ...
        # while not alpha.allclose(end_focus) and len(result) < target.shape[-2]:

        while len(result) < target.shape[-2]:
            query = torch.cat([alpha, hid], dim=-1)
            alpha = (self.attention(query).unsqueeze(-1) * encoded).sum(1)
            # hid = self.decoder(torch.cat([alpha, out], dim=-1), hid)
            hid = self.decoder(alpha, hid)
            out = hid
            for fc in self.de2voc_mlp:
                out = hid + fc(out)
            if self.do_maskedLM:
                out = F.log_softmax(out, dim=-1) # for cross-entropy 
            result.append(out)
...
if __name__ == "__main__":
    ...
    if test_attn:
        ...
        print('Forward Propagation for Attention ran without error. ')
    ...
    if test_char2v:
        ...
        print('Char2Voc encoder-decoder with Attention ran without error. ')
